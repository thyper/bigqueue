...
describe("Open stack http api",function(){
    ...
    describe("Exchange Messages",function(done){
        beforeEach(function(done){
            bqClient.createTopic("test-tenant-testTopic",function(err){
                should.not.exist(err)
                bqClient.createTopic("test-tenant-testTopic2",function(err){
                    should.not.exist(err)
                    bqClient.createConsumerGroup("test-tenant-testTopic","test-tenant1-testConsumer1",function(err){
                        should.not.exist(err)
                        bqClient.createConsumerGroup("test-tenant-testTopic","test-tenant2-testConsumer2",function(err){
                            should.not.exist(err)
                            bqClient.createConsumerGroup("test-tenant-testTopic2","test-tenant2-testConsumer2",function(err){
                                should.not.exist(err)
                                done()
                            })
                        })
        ...
        it("should get an error if a post message receive an invalid json",function(done){
            request({
                url:"http://127.0.0.1:8080/test-tenant/messages",
                method:"POST",
                body:"foo" 
        ...
        it("should receive posted messages on multi-topic post",function(done){
            request({
                uri:"http://127.0.0.1:8080/test-tenant/messages",
                method:"POST",
                ...
                request({
                    uri:"http://127.0.0.1:8080/test-tenant/topics/testTopic/consumers/test-tenant1/testConsumer1/messages",
                    method:"GET",
                    json:true
                },function(error,response,body){
                    should.exist(response)
                    ...
                    request({
                        uri:"http://127.0.0.1:8080/test-tenant/topics/testTopic2/consumers/test-tenant2/testConsumer2/messages",
                        method:"GET",
                        json:true
                    },function(error,response,body){
                        should.exist(response)
        ...
        it("should get an error if we try to get messages from non existent topic",function(done){
            request({
                uri:"http://127.0.0.1:8080/test-tenant/topics/testTopic-dsadsa/consumers/test-tenant1/testConsumer1/messages",
                method:"GET",
                json:true
            },function(error,response,body){
                response.statusCode.should.equal(400)
        ...
        it("should can get the same message if there are 2 consumer groups",function(done){
            request({
                ...
                request({
                    ...
                    body.msg.should.equal("testMessage")
                    request({
                        uri:"http://127.0.0.1:8080/test-tenant/topics/testTopic/consumers/test-tenant2/testConsumer2/messages",
                        method:"GET",
                        json:true
                    },function(error,response,body){
                        should.exist(response)
        ...
        it("should receive the same message if the visibility window is rached",function(done){
            request({
                ...
                var postId = body[0].id
                request({
                    uri:"http://127.0.0.1:8080/test-tenant/topics/testTopic/consumers/test-tenant1/testConsumer1/messages?visibilityWindow=1",
                    method:"GET",
                    json:true
                },function(error,response,body){
                    should.exist(response)
        ...
        it("should enable to do a DELETE of a message so this message shouldn't be received another time",function(done){
            request({
                ...
                request({
                    ...
                    body.msg.should.equal("testMessage")
                    request({
                        uri:"http://127.0.0.1:8080/test-tenant/topics/testTopic/consumers/test-tenant1/testConsumer1/messages/"+body.recipientCallback,
                        method:"DELETE",
                        json:true
    ...
    describe("Limits",function(){
        it("should get an error if a posted message have more than 64kb",function(done){
            ...
            var msg = b64kb()
            request({
                uri:"http://127.0.0.1:8080/test-tenant/topics/testTopic/messages",
                method:"POST",
                body:msg,
