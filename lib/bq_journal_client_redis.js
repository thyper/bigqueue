var redis = require('simple_redis_client'),
...
BigQueueJournalClient.prototype.write = function(node,topic, messageId, message, ttl,callback){
    ...
    this.redisClient.execute("eval",redisScripts["journalWrite"],0,node,topic,messageId,JSON.stringify(message),ttl,function(err,data){
        timer("Journal Write")
        if(err){
...
BigQueueJournalClient.prototype.retrieveMessages = function(node,topic, idFrom, callback){
    this.redisClient.execute("eval",redisScripts["journalRetrieve"],0,node,topic, idFrom, function(err,data){
        if(err){
            if(callback)
                callback(err)
        }else{
