...
BigQueueClusterClient.prototype.nodeAdded = function(node){
    ...
    clusterNode.client.on("error",function(err){
      log.log("error","Error on node", err)
    })
    log.log("info", "Node added [ %j ]", clusterNode.id)
...
BigQueueClusterClient.prototype.nodeRemoved = function(node){
   ...
   var clusterNode = this.getClientById(this[meta.clientsList],node.id)
    ...
    log.log("info", "Node removed [%s]", node.id)
}
...
BigQueueClusterClient.prototype.nodeDataChange = function(node){
  ...
  if(!clusterNode){
    ...
    log.log("info", "Node data chaged [%j]", node)
...
BigQueueClusterClient.prototype.withSomeClient = function(run,cb){
    ...
    function monitor(){
        ...
        setTimeout(function(){
            if(!m.emited){
                jsdog.recordSimpleMetric("application.bigqueue.web_node.timeouts",1, self.baseTags.concat(["cluster_node:"+actualClient.id]));
                log.log("info", "Timeout on client: "+actualClient.id)
    ...
    this.withSomeClient(
        function(clusterNode,monitor){
            ...
            if(clusterNode.data.read_only){
                log.log("debug", "Node ["+JSON.stringify(clusterNode.data)+"] is in readonly")
                return monitor.next()
            }
            ...
            clusterNode.client.postMessage(topic,message,function(err,key){
                ...
                if(!err){
                  ...
                  monitor.emit(err,key)
                  log.log("error", "Error posting message [%j] [%j] [topic:%s]", err, clusterNode.data, topic)
                }
            })
        },
        function(err,key, workClient) {
            jsdog.recordSimpleMetric("application.bigqueue.topics.stats.create_messages",1, self.baseTags.concat(["topic:"+topic,"cluster_node:"+(workClient ? workClient.id : "error")]));
            if(err) {
              jsdog.recordSimpleMetric("application.bigqueue.topics.stats.create_message_errors",1, self.baseTags.concat(["topic:"+topic]));
            }
    ...
    if(!clusterNode.data.journals || clusterNode.data.journals.length == 0 ){
        ...
        for(var i in journalIds){
            ...
            if(!journal){
                ...
                log.log("error", "Error getting journal "+ err);
                err = {"msg":err}
...
BigQueueClusterClient.prototype.getMessage = function(topic,group,visibilityWindow,callback){
    ...
    this.withSomeClient(
        ...
        function(err,data, workClient){
          ...
          jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.get_messages",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group, "cluster_node:"+(workClient ? workClient.id : "error")]));
          if(data === undefined) {
            jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.get_messages_empty",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group, "cluster_node:"+(workClient ? workClient.id : "error")]));
          } else {
            jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.get_messages_with_content",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group, "cluster_node:"+(workClient ? workClient.id : "error")]));
          }
          if(err){
              jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.get_message_errors",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
              log.log("error", "Error getting messages ["+err.msg+"]")
              err = {"msg":err.msg}
          }
...
BigQueueClusterClient.prototype.ackMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.ackMessage(topic,group,repData.id,function(err){
        ...
        try {
          jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.ack_messages",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group,"cluster_node:"+node.id]));
        }catch(e) {
          console.log(e);
        }
        if(err){
            jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.ack_message_errors",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group,"cluster_node:"+node.id]));
            log.log("error", "Error doing ack of recipientCallback ["+recipientCallback+"], error: "+err.msg)
            err = {"msg":err.msg}
            return callback(err)
        }
        callback()
...
BigQueueClusterClient.prototype.failMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.failMessage(topic,group,repData.id,function(err){
        ...
        jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.fail_message_action",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group,"cluster_node:"+node.id]));
        if(err){
            err = {"msg":err}
            log.log("error", "Error doing fail of recipientCallback [%s], error: %j", recipientCallback, err)
        }
        callback(err)
    })
}
...
BigQueueClusterClient.prototype.listTopics = function(callback){
    ...
    this.zkClient.a_get_children(topicsPath,false,function(rc,error,children){
        if(rc!=0){
            log.log("error", "Error listing topics [%j]", error)
        }
        callback(children)
    })
}
...
BigQueueClusterClient.prototype.getConsumerGroups = function(topic,callback){
    ...
    this.zkClient.a_exists(consumerGroupsPath,false,function(rc,error){
        ...
        self.zkClient.a_get_children(consumerGroupsPath,false,function(rc,error,children){
            if(rc!=0){
                log.log("error", "Error getting consumer groups [%j]", error)
