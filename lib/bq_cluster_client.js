 ...
 */
function BigQueueClusterClient(zkClient,zkClusterPath,zkFlags,createClientFunction,createJournalFunction,sentCacheTime,refreshTime){
    this.nodes=[]
    this.journals=[]
...
BigQueueClusterClient.prototype.createTopic = function(){
    ...
    this.zkClient.a_exists(topicPath,false,function(rc,error,stats){
            if(rc == ZK.ZNONODE){
                ...
                callback("Error topic ["+msg+"] already exist")
            }
    })
}
...
BigQueueClusterClient.prototype.deleteTopic = function(topic,callback){
    ...
    this.zkClient.a_exists(topicPath,false,function(rc,error,stats){
        if(rc!=0){
            return callback("Topic doesn't exist ["+topic+"]")
        }
        self.zkClient.a_get_children(topicConsumerPath,false,function(rc,error,children){
            if(rc!=0)
                return callback("Error reading from zookeeper ["+topicsConsumerPath+"], "+rc+"-"+error)
            if(children.lenght > 0)
                return callback("Topic ["+topic+"] contains consumers")
                ...
                if(err){
                    ...
                    callback(err)
                     ...
                     self.zkClient.a_delete_(topicConsumerPath,-1,function(rc, error){
                        if(rc!=0){
                            log.err("Creating path in zookeeper ["+topicConsumerPath+"],error: "+error)
                            callback("Error registering topic ["+topic+"] into zookeeper")
                        }else{
                            self.zkClient.a_delete_(topicPath,-1,function(rc, error){
                                if(rc!=0){
                                    log.err("Creating path in zookeeper ["+topicPath+"], error: "+log.pretty(error))
                                    callback("Error registering topic ["+topic+"] into zookeeper")
                                }else{
                                    callback()
                                }
...
BigQueueClusterClient.prototype.getConsumerGroups = function(topic,callback){
    ...
    this.zkClient.a_exists(consumerGroupsPath,false,function(rc,error){
        if(rc != 0){
            callback("Path ["+consumerGroupsPath+"], doesn't exists")
            return
        }
