 ...
 */
function BigQueueClusterClient(zkClient,zkClusterPath,zkFlags,createClientFunction,createJournalFunction,sentCacheTime,refreshTime,clientTimeout){
    this.nodes=[]
    this.journals=[]
    ...
    this.clientTimeout = 125 || clientTimeout
    this.hostname = os.hostname()
    this.uid = 0
    this.sentCache = {}
    this.controlZookeeper = false
    //If the property is not set we'll use 10s of refresh time
    ...
    function monitor(){
        ...
        this.emit = function(err,data){
            ...
            if(calls < self.nodes.length){
                ...
                var error = err
                m.timer("Finishing monitor exec with errors")
...
BigQueueClusterClient.prototype.getMessage = function(topic,group,visibilityWindow,callback){
    ...
    this.withSomeClient(
        function(clusterNode,monitor){     
            if(!clusterNode || (clusterNode.data.status != "UP")){
                monitor.emit({err:"Cluster node error or down"})
                return
            }
