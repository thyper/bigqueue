    ...
    if(!clusterNode.data.journals || clusterNode.data.journals.length == 0 ){
        ...
        for(var i in journalIds){
            ...
            if(!journal){
                ...
                log.log("error", "Error getting journal [cluster_node:"+clusterNode.id+"]"+ err);
                err = {"msg":err}
                return cb(err,undefined)
            }
            if(journal.data.status != "UP"){
                var err = "Error journal [journal_id:"+journalIds[i]+"] DOWN" 
                log.log("error", err)
                err = {"msg":err}
                return cb(err,undefined)
            }
...
BigQueueClusterClient.prototype.ackMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.ackMessage(topic,group,repData.id,function(err){
        ...
        if(err){
            ...
            log.log("error", "Error doing ack of recipientCallback [recipient_callback:"+recipientCallback+"], error: "+err.msg)
            err = {"msg":err.msg}
            return callback(err)
        }
        callback()
