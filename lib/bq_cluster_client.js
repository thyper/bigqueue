...
function BigQueueClusterClient(zkClient,zkClusterPath,zkFlags,createClientFunction,createJournalFunction,sentCacheTime,refreshTime,clientTimeout,statsInterval,statsFile){
    ...
    this.refreshTime = refreshTime || 60000
...
BigQueueClusterClient.prototype.nodeAdded = function(node){
    ...
    if(!clusterNode){
        ...
        clusterNode.client.on("error",function(err){
            log.err(err)
          /**
           * We won't notify errors from nodes, instead of we'll check nodes only on orchestrator
           * FROM VERSION 0.2.0
           */
//            if(clusterNode && clusterNode.data.status!="DOWN")
 ...
 *       this.zkClient.a_exists(nodePath,false,function(rc,error,stat){
 *           var listName = self.clientsMeta[self.nodesPath].clientsList
 *           var nodesList = self[listName]
 *           //The node was deleted
 *           if(rc!=0){
 *                nodesList = nodesList.filter(function(val){
 ...
 *               if(node.data){ 
 *                   node.data.errors++
 *                   self.zkClient.a_set(nodePath,JSON.stringify(node.data),-1,function(rc,error,stat){
 *                       if(rc!=0){
...
/**
 */
BigQueueClusterClient.prototype.rotateArrayAndGetElement = function(arr){
 ...
 */
BigQueueClusterClient.prototype.withSomeClient = function(run,cb,clients){
    var nodes = clients || this.nodes
    var self = this
    var calls = 0
    var actualClient
    function monitor(){
        ...
        this.emit = function(err,data,finishWithNoData){
            ...
            if(calls < self.nodes.length){
                actualClient = self.rotateArrayAndGetElement(nodes)
                run(actualClient,new monitor())
            }else{
                var error = err
                m.timer("Finishing monitor exec with errors ["+error+"] ")
                if(error)
                    cb({err:"Node execution fail ["+error+"]"},undefined)
                else
                    cb()
            }
        }
    ...
    actualClient = this.rotateArrayAndGetElement(nodes)
    run(actualClient,new monitor())
}
...
/**
 * This function will return the besto nodes to do get's for an specific consumer
 */
BigQueueClusterClient.prototype.getBestNodes = function(topic,consumer){
            ...
            clusterNode.client.getMessage(topic,group,visibilityWindow,function(err,data){
                ...
                if(err){
                    ...
                    if(data.id){
                        ...
                        monitor.emit(undefined,data)
                    }else{
                        monitor.emit(undefined,undefined)
                    }
                }
            })
        },
