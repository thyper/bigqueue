...
BigQueueClusterClient.prototype.nodeRemoved = function(node){
    ...
    if(node.path == this.nodesPath && clusterNode){
    ...
    this.withSomeClient(
        function(clusterNode,monitor){
                ...
                if(!err){
                    ...
                    //Load to journal
                    self.writeMessageToJournal(key.id,message,clusterNode,function(err){
                        monitor(err,key)
                    })
...
BigQueueClusterClient.prototype.writeMessageToJournal = function(msgId, message, clusterNode, cb){
    if(!clusterNode.data.journals || clusterNode.data.journals.length == 0 ){
        ...
        this.withEvery(journals,
            function(journal, jMonitor){
                journal.client.write(clusterNode.id,msgId,message,undefined,function(err){
                    jMonitor(err)
                })
            },
            function(err,data){
