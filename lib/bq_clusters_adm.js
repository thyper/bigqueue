...
BigQueueClustersAdmin.prototype.updateNodeMetrics = function(cluster,node,stats, callback) {
  ...
  try {
    if(node.node_stats) {
      var n_stats = node.node_stats;
      jsdog.recordCompoundMetric("application.bigqueue.cluster_node.user_memory_b", n_stats.used_memory && parseInt(n_stasts.used_memory));
      jsdog.recordCompoundMetric("application.bigqueue.cluster_node.used_cpu_sys", n_stats.used_cpu_sys && parseFloat(n_stasts.used_cpu_sys));
      jsdog.recordCompoundMetric("application.bigqueue.cluster_node.used_cpu_user", n_stats.used_cpu_user && parseFloat(n_stasts.used_cpu_user));
      jsdog.recordCompoundMetric("application.bigqueue.cluster_node.ops_per_sec", n_stats.instantaneous_ops_per_sec && parseInt(n_stasts.instantaneous_ops_per_sec));
    }
  }catch(e) {
    //Fail metric should not fail process
    log.log("error","Error recording node metric %s %j", node_tags,e);
  }
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", err);
    ...
    mysqlConn.beginTransaction(function(err) {
      ...
      async.each(stats.topics_stats, function(topic_stats, tCb) {
        ...
        //We'll use max to show the value
        try {
          jsdog.recordFullMetric("application.bigqueue.topics.stats.head",topic_stats.topic_head, topic_tags);
        } catch(e) {
          //Fail metric should not fail process
          log.log("error","Error recording topic metric %s %j",topic_stats.topic_head, topic_tags,e);
        }
        ...
        async.each(topic_stats.consumers, function(consumer,cCb) {
          ...
          try {
            jsdog.recordCompoundMetric("application.bigqueue.consumers.stats.lag",stats.lag, consumer_tags);
            jsdog.recordCompoundMetric("application.bigqueue.consumers.stats.fails",stats.fails, consumer_tags);
            jsdog.recordCompoundMetric("application.bigqueue.consumers.stats.processing",stats.processing, consumer_tags);
          } catch(e) {
