...
BigQueueClustersAdmin.prototype.getTopicStats = function(topic,cluster,callback){
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", "Error getting connection from mysql on getTopicStats", err);
      if(mysqlConn) {
        mysqlConn.release();
      }
      return callback(err);
...
BigQueueClustersAdmin.prototype.getEndpoinsForCluster = function(cluster, callback) {
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", "Error getting connection from mysql on getEndpointsForCluster", err);
      return callback(err);
    }
...
BigQueueClustersAdmin.prototype.getTopicData = function(topic,callback){
    ...
    this.mysqlPool.getConnection(function(err, mysqlConn) {
      if(err) {
        log.log("error", "Error getting connection from mysql on getTopicData", err);
        return callback(err);
      }
      async.series([
        //Base topic data
...
BigQueueClustersAdmin.prototype.listClusters = function(callback){
  ...
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", "Error getting connection from mysql on listClusters", err);
      return callback(err);
    }
...
BigQueueClustersAdmin.prototype.getNodesByCluster = function(cluster, callback) {
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", "Error getting connection form mysql on getNodesByCluster",err);
      return callback(err);
    }
...
BigQueueClustersAdmin.prototype.getNodeDataByType = function(cluster,node,type,callback){
  ...
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", "Error getting connection from mysql on getNodeDataByType" err);
      return callback(err);
    }
    var node_data;
...
BigQueueClustersAdmin.prototype.updateNodeMetrics = function(cluster,node,stats, callback) {
  ...
  var node_tags = ["cluster:"+cluster, "cluster_node:"+node];
  try {
    if(stats.node_stats) {
      var n_stats = stats.node_stats;
      jsdog.recordPlainGauge("application.bigqueue.cluster_node.user_memory_b", n_stats.used_memory && parseInt(n_stats.used_memory), node_tags);
      jsdog.recordPlainGauge("application.bigqueue.cluster_node.used_cpu_sys", n_stats.used_cpu_sys && parseFloat(n_stats.used_cpu_sys), node_tags);
      jsdog.recordPlainGauge("application.bigqueue.cluster_node.used_cpu_user", n_stats.used_cpu_user && parseFloat(n_stats.used_cpu_user), node_tags);
      jsdog.recordPlainGauge("application.bigqueue.cluster_node.ops_per_sec", n_stats.instantaneous_ops_per_sec && parseInt(n_stats.instantaneous_ops_per_sec), node_tags);
    }
  }catch(e) {
  ...
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", "Error getting connection from mysql on uodateNodeMetric", err);
      if(mysqlConn) {
        mysqlConn.release();
      }
      return callback(err);
...
BigQueueClustersAdmin.prototype.getTasksByCriteria = function(criteria, callback) {
  ...
  build_criteria_query(criteria, function(query_filter ,criteria_arr) {
   self.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", "Error getting connection from mysql on getTasksByCriteria", err);
      if(mysqlConn) {
        mysqlConn.release();
      }
      return callback(err);
...
BigQueueClustersAdmin.prototype.updateTaskStatus = function(task_id, task_status, callback) {
   this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      log.log("error", "Error getting connection from mysql on updataTaskStatus", err);
      return callback(err);
    }
