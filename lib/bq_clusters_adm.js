  ...
  self.mysqlPool.getConnection(function(err, mysqlConn) {
    ...
    async.series([
      ...
      function(cb) {
        ...
        mysqlConn.query("INSERT INTO consumers (consumer_id, tenant_id, tenant_name, consumer_name, topic_id, create_time) VALUES (?,?,?,?,?,now())",
  ...
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    ...
    mysqlConn.beginTransaction(function(err) {
      ...
      async.each(stats.topics_stats, function(topic_stats, tCb) {
        ...
        if(!topic_id || !topic_stats.consumers || !topic_stats.consumers.length) {
