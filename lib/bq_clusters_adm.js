...
BigQueueClustersAdmin.prototype.updateNodeMetrics = function(cluster,node,stats, callback) {
  ...
  try {
    if(stats.node_stats) {
      ...
      jsdog.recordPlainGauge("application.bigqueue.cluster_node.user_memory_b", n_stats.used_memory && parseInt(n_stats.used_memory));
      jsdog.recordPlainGauge("application.bigqueue.cluster_node.used_cpu_sys", n_stats.used_cpu_sys && parseFloat(n_stats.used_cpu_sys));
      jsdog.recordPlainGauge("application.bigqueue.cluster_node.used_cpu_user", n_stats.used_cpu_user && parseFloat(n_stats.used_cpu_user));
      jsdog.recordPlainGauge("application.bigqueue.cluster_node.ops_per_sec", n_stats.instantaneous_ops_per_sec && parseInt(n_stats.instantaneous_ops_per_sec));
    }
  }catch(e) {
