        ...
        async.each(topics_db, function(topic, cb_each) {
          self.getConsumerByCriteria({"t.topic_id":topic.topic_id}, function(err, data) {
            if(err) {
              ...
              topics.push({topic_id: topic.topic_id, cluster: topic.cluster, ttl: topic.ttl, consumers: consumers_arr});
              cb_each();
            }
          })
        }
    ...
    async.series([
      ...
      function(cb) {
        async.each(tasks, function(task, sqlCb) {
          ...
          mysqlConn.query("INSERT INTO tasks (data_node_id,task_group,task_type, task_data) VALUES (?,?,?,?)", [task.data_node_id,uuid, task.task_type, JSON.stringify(task.task_data)],sqlCb);
        },function(err) {
          cb(err);
        });
      }
...
BigQueueClustersAdmin.prototype.getTasksByCriteria = function(criteria, callback) {
  var self = this;
  var criteria_arr = [];
  var query = "SELECT task_id, data_node_id, task_group, task_type, task_data, status FROM tasks";
...
BigQueueClustersAdmin.prototype.updateTask = function(task, callback) {
}
