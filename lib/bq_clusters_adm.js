...
BigQueueClustersAdmin.prototype.updateGenericNodeData = function(clusterName,node,type,callback){
    var self = this
    var nodePath = this.zkClustersPath+"/"+clusterName+"/"+type+"/"+node.name
    this.zkClient.a_get(nodePath,false,function(rc,error,stat,data){
        ...
        if(!node.config) {
          return callback({msg: "config property not found"});
        }
        ...
        if(!utils.checkNodeData(nodeData)){
            return callback({"msg":"Node "+type+" uncomplete [ "+JSON.stringify(nodeData)+" ] [host,port,status,journals are required]"})
        }
        self.zkClient.a_set(nodePath,JSON.stringify(nodeData),-1,function(rc,error, stat){
            if(rc != 0){
                callback({"msg":"Error ["+rc+" - "+error+"] updating data of "+type+" on path:"+nodePath})
            }else{
                callback()
            }
        })
