...
BigQueueClusterOrchestrator.prototype.topicsController = function(node,type,rule,context){
    if(type == ADDED){
        context.cluster["topics"][node.node] = {"data":node.data}
...
BigQueueClusterOrchestrator.prototype.consumerGroupsController = function(node,type,rule,context){
    ...
    if(type == ADDED){
       ...
       delete context.cluster["topics"][topic]["consumerGroups"][node.node]
   }
}
...
BigQueueClusterOrchestrator.prototype.nodesController = function(node,type,rule,context){
    ...
    if(type == ADDED){
        context.cluster["nodes"][node.node] = JSON.parse(node.data)
        context.healthCheckNode(node.node,data,function(err){
...
BigQueueClusterOrchestrator.prototype.journalsController = function(node,type,rule,context){
    ...
    if(type == ADDED){
        context.cluster["journals"][node.node] = JSON.parse(node.data)
        context.healthCheckJournal(node.node,data,function(err){
...
BigQueueClusterOrchestrator.prototype.healthCheckJournal = function(journalId,data,callback){
    ...
    this.checkClientConnection(this.createJournalClientFunction,data,function(err,client){
        ...
        self.updateClientStatus("journals",journalId,data,err,function(){
            if(err){
                var keys = Object.keys(self.cluster["nodes"]).filter(function(nodeId){
                    var nodeData = self.cluster["nodes"][nodeId]
                ...
                for(var i in keys){
                    var nodeId = keys[i]
                    var node = self.cluster["nodes"][nodeId]
                    node.status = "DOWN"
                    self.updateClientStatus("nodes",nodeId,node,"Journal down",function(){})
                }
            }
            callback()
        })
