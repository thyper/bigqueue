...
TasksJob.prototype.doProcess = function(callback) {
  ...
  needle.get(this.pendingTasksUrl, function(err, response) {
    ...
    if(response.statusCode != 200) {
      log.error("Error getting data from tasks [url: "+self.pendingTasksUrl+"] [status: "+response.statusCode+"] [response: "+response.body+"]")
    ...
    async.eachSeries(tasks, function(task, cb) {
      async.series([
        function(sCb) {
          log.info("Procesando: ",JSON.stringify(task));
        ...
        function(sCb) {
          needle.put(self.tasksUrl+"/"+task.task_id,{task_status: "FINISHED"}, {json:true}, function(err, resp) {
            ...
            if(response.statusCode > 299) {
              log.error("Error changing status of ["+task.stas_id+"] [status: "+response.statusCode+"] [response: "+response.body+"]");
              return sCb({msg: "Error changing status of taskId ["+task.task_id+"]"});
            }
            log.info("Estado de tarea finalizado correctamente ["+self.tasksUrl+"/"+task.task_id+"]");
            sCb();
          }); 
        }
      ], cb);
