        ...
        this.redisClient.on("error",function(err){
            ...
            process.nextTick(function(){
                try{
                    ...
                    log.err("Error emmiting error",e)
                }
            })
        })
...
BigQueueClient.prototype.deleteTopic = function(topic,callback){
    this.redisClient.eval(redisScripts["deleteTopic"],0,topic,function(err,data){
        if(err){
...
BigQueueClient.prototype.deleteConsumerGroup = function(topic,consumerGroup,callback){
    this.redisClient.eval(redisScripts["deleteConsumer"],0,topic,consumerGroup,function(err,data){
        if(err){
...
BigQueueClient.prototype.resetConsumerGroup = function(topic, consumerGroup,callback){
    this.redisClient.eval(redisScripts["resetConsumerGroup"],0,topic,consumerGroup,function(err,data){
        if(err){
        ...
        if(err){
            ...
            if(callback){
                ...
                if(data.lag != undefined){
                    self.lags[topic+"-"+consumer] = data.lag
                    delete data["lag"]
                }
...
BigQueueClient.prototype.getTopicTtl = function(topic,callback){
    this.redisClient.get("topics:"+topic+":ttl",function(err,data){
        if(err){
            err = {"msg":""+err}
            callback(err)
        }else{
...
/**
 * The difference from the getConsumerStats is that this function
 * is synchronous and based on local (cached) information to give the 
 * capability to che cluster client of balance the load based on the amount 
 * of messages
 */
BigQueueClient.prototype.getConsumerLag = function(topic, consumer){
    return this.lags[topic+"-"+consumer] 
}
